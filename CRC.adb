...
package body CRC is
   ...
   ---------------------------------------------------------------------
   -- Generate the CRC lookup table for the CRC polynomial given in Name
   ---------------------------------------------------------------------
   procedure Generate_Table(Name : Crc_Name_T := CRC_CCITT) is
   ...
   begin
      for Dividend in Crc_Table_Range'First .. Crc_Table_Range'Last loop
         Remainder := Shift_Left(Unsigned_32(Dividend), Width -8);
         ...
         for Bit in 1 .. 8 loop
           ...
           Crc_Table(Dividend) := Remainder and CrC_Name(Name).Mask; -- !!
      end loop;
   end Generate_Table;
   ...
   begin
      for I in 1 .. Message'Length loop
         ...
         Data := Unsigned_32_To_Unsigned_8(Reflect_Val xor Shift_Right(Remainder,(Width-8)));
        Remainder := (Crc_Table(Data) xor Shift_Left(Remainder, 8)) and Crc_Name(Name).Mask;
